...
export class PoolApp {
  ...
  focusCameraOnWall(wall) {
    ...
    let tangent2 = null;
    let wallLength = Math.max(wallSize.x, wallSize.y, 0.5);
    if (posAttr && posAttr.count >= 2) {
      ...
      if (tangent2.lengthSq() > 1e-8) {
        ...
        for (let i = 0; i < posAttr.count; i++) {
          const wp = new THREE.Vector3(posAttr.getX(i), posAttr.getY(i), posAttr.getZ(i)).applyMatrix4(wall.matrixWorld);
          const proj = wp.x * tangent2.x + wp.y * tangent2.y;
          if (proj < minProj) minProj = proj;
          if (proj > maxProj) maxProj = proj;
        }
        wallLength = Math.max(0.5, maxProj - minProj);
      } else {
        tangent2 = null;
      }
    }
    ...
    if (!tangent2) {
      ...
      wallLength = Math.max(0.5, dx >= dy ? dx : dy);
    }
    ...
    const outline = Array.isArray(this.poolGroup.userData?.outerPts)
    ...
    if (outline?.length >= 3) {
      const probeDist = Math.max(0.25, Math.min(0.8, Math.max(wallSize.x, wallSize.y, wallLength) * 0.25));
      const plusProbe = new THREE.Vector2(
        wallCenter.x + inward2.x * probeDist,
        wallCenter.y + inward2.y * probeDist
      );
      const minusProbe = new THREE.Vector2(
        wallCenter.x - inward2.x * probeDist,
        wallCenter.y - inward2.y * probeDist
      );
      const plusInside = pointInPolygon(plusProbe, outline);
      const minusInside = pointInPolygon(minusProbe, outline);
      if (plusInside === false && minusInside === true) {
    ...
    if (toPoolCenter2.lengthSq() > 1e-8 && inward2.dot(toPoolCenter2) < 0) {
      const candidate = inward2.clone().multiplyScalar(-1);
      if (!outline?.length) {
        inward2.copy(candidate);
      } else {
        const probeDist = Math.max(0.25, Math.min(0.8, Math.max(wallSize.x, wallSize.y, wallLength) * 0.25));
        const candidateProbe = new THREE.Vector2(
          wallCenter.x + candidate.x * probeDist,
          wallCenter.y + candidate.y * probeDist
        );
        if (pointInPolygon(candidateProbe, outline) !== false) {
          inward2.copy(candidate);
        }
      }
    }
    ...
    const halfVFov = THREE.MathUtils.degToRad(this.camera.fov * 0.5);
    const halfHFov = Math.atan(Math.tan(halfVFov) * this.camera.aspect);
    const fitHorizontal = (wallLength * 0.62) / Math.max(Math.tan(halfHFov), 0.01);
    const fitVertical = (Math.max(wallSize.z * 1.35, 1.25) * 0.5) / Math.max(Math.tan(halfVFov), 0.01);
    const poolSize = poolBounds.getSize(new THREE.Vector3());
    const standoff = Math.max(
      1.6,
      fitHorizontal,
      fitVertical,
      Math.min(Math.max(poolSize.x, poolSize.y) * 0.22, 3.8)
    );
    const eyeHeight = Math.max(0.85, Math.min(1.6, wallBounds.min.z + wallSize.z * 0.58));
    const targetHeight = Math.max(-0.15, Math.min(0.65, wallBounds.min.z + wallSize.z * 0.42));
...
// --------------------------------------------------------------
  ...
  _applyWallExtraToMeshesFromKey(key, extra = 0) {
    ...
    walls.forEach((wall) => {
      ...
      let seg = null;
      let segKey = null;
      if (Array.isArray(copingSegments) && wall.userData?.copingIndex != null) {
        segKey = wall.userData.copingIndex;
        seg = copingSegments[segKey] || null;
      } else if (copingSegments && typeof copingSegments === "object") {
        if (wall.userData?.copingIndex != null && copingSegments[wall.userData.copingIndex]) {
          segKey = wall.userData.copingIndex;
          seg = copingSegments[segKey];
        } else if (wall.userData?.side && copingSegments[wall.userData.side]) {
          segKey = wall.userData.side;
          seg = copingSegments[segKey];
        }
      }
      ...
      if (seg) {
        ...
        movedCoping.add(segKey);
      }
    });

    const copingRing = this.poolGroup.userData?.copingMesh;
    if ((!copingSegments || (Array.isArray(copingSegments) && !copingSegments.length)) && copingRing) {
...
// PBR / Caustics integration should never hard-crash the app if a module fails
  ...
  _isEditableCustomLShape() {
    return !!(
      this.isCustomShape &&
      this.baseShapeType === "L" &&
      this.editablePolygon?.vertices?.length === 6
    );
  }

  _applyCustomLShapeNotchToEditablePolygon() {
    if (!this._isEditableCustomLShape()) return false;

    const verts = this.editablePolygon.vertices;
    const bbox = verts.reduce((acc, v) => {
    ...
    const length = Math.max(0.01, bbox.maxX - bbox.minX);
    const width = Math.max(0.01, bbox.maxY - bbox.minY);

    const notchL = THREE.MathUtils.clamp(
      length * (Number.isFinite(this.poolParams.notchLengthX) ? this.poolParams.notchLengthX : 0.4),
      0.6,
      Math.max(0.6, length - 0.6)
    );
    const notchW = THREE.MathUtils.clamp(
      width * (Number.isFinite(this.poolParams.notchWidthY) ? this.poolParams.notchWidthY : 0.45),
      0.6,
      Math.max(0.6, width - 0.6)
    );

    const innerX = bbox.maxX - notchL;
    const innerY = bbox.maxY - notchW;
    ...
    this.editablePolygon._emitChange?.();
    return true;
  }
      ...
      slider.addEventListener("input", (e) => {
        ...
        if ((id === "length" || id === "width") && this.isPolygonShape()) {
          try {
            ...
            if (this._isEditableCustomLShape()) {
              this._applyCustomLShapeNotchToEditablePolygon();
            }
            if (this.poolParams.shape === "freeform" && this.editablePolygon) {
              this.editablePolygon.isRectangular = false;
            }
          } catch (_) {}
        } else if ((id === "notchLengthX" || id === "notchWidthY") && this._isEditableCustomLShape()) {
          try { this._applyCustomLShapeNotchToEditablePolygon(); } catch (_) {}
        }
